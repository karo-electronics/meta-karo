...
case "$ACTION" in
	add|"")
		...
		if [ "${DEVBASE}" = "${MDEV}" ] ; then
			# check for subdirectories that represent disk partitions
			# avoiding 'boot#' and 'rpmb#' HW partitions on eMMC
			shopt -s nullglob
			for p in /sys/block/${DEVBASE}/${DEVBASE}p[0-9]*;do
